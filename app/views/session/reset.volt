        ...
        <div class="card-body">
            <div class="row">
                <div class="col">
                    {{ flash.output() }}
                    {% if result == true %}
                        <p>{{t._("recovery-password-msg")}}</p>
                        <div class="text-left alert alert-warning">
                            <div style="margin-bottom: 5px">Пароль должен быть минимум 8 символов.</div>
                            <div style="margin-bottom: 5px">Обязательный верхний и нижний регистр, цифра, и специальные символы (! " # $ % & \' ( ) * + , - . / : ; < = > ? @ [ \\ ] ^ _` { | } ~).</div>
                            <div>Не рекомендуется использовать пароль, совпадающий с паролем от электронной цифровой подписи (ЭЦП)</div>
                        </div>
                        <form id="frm_restore" class="animated fadeIn" method="POST" action="/session/reset_password/{{ md }}/{{ id }}">
                            ...
                            <div class="form-group">
                                <div class="controls">
                                    <button type="submit" name="forgot_submit" class="btn btn-primary">{{t._("change-password")}}</button>
                                </div>
                            </div>
                        </form>
                    {% elseif result == false %}
                        <p>{{t._("recovery-password-false")}}</p>
