...
<div class="card mt-1">
    ...
    <div class="card-body">
        <form method="POST" action="/moderator_order">
            ...
            <div class="row mt-2">
                ...
                <div class="col-auto mt-4">
                    ...
                    <a href="/moderator_order/clearFilter" class="btn btn-warning">{{ t._('Сбросить') }}</a>
                </div>
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-sm" id="order_list_table">
                ...
                <tr>
                    ...
                    <th>Исполнитель</th>
                    <th style="width:12%">{{ t._('Операции') }}</th>
                </tr>
                </thead>
                <tbody>
                {% if items|length %}
                    {% for it in items %}
                        <tr class="{{ it.row_class }}">
                            ...
                            <td class="v-align-middle"><a href="/moderator_order/view/{{ it.p_id }}" target="_blank">{{ it.name_line ? it.name_line : 'Нет данных' }}</a></td>
                            ...
                            <td class="v-align-middle">{{ t._(it.approve_label) }}</td>
                            <td class="v-align-middle">
                                <div class="order-status">
                                    <div class="order-executor" id="order-{{ it.p_id }}">
                                        Исполнитель: <b style="font-size: 12px">{{ it.executor_name }}</b>
                                    </div>
                                </div>
                            </td>
                            <td class="v-align-middle">
                                <a href="/moderator_order/download_zip/{{ it.p_id }}" title="{{ t._('download') }}" class="btn btn-primary btn-sm" target="_blank">
                                    <i data-feather="star" width="14" height="14"></i> Скачать все
                                </a>
                            </td>
                        </tr>
                    {% endfor %}
