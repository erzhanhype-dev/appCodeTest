...
<form action="/create_order/good_edit/{{ good.id }}" method="post" id="frm_goods">
      ...
      <div class="card mt-3">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <div class="controls">
              <input type="text" name="good_basis" id="good_basis" class="form-control" maxlength="50" value="{{ good.basis }}" required>
            </div>
          </div>
          <div class="form-group">
          ...
          <label class="form-label"><b>{{ t._("car-calculate-method") }}</b></label>
          <div class="controls">
            <div class="btn-group btn-group-toggle" data-toggle="buttons">
              <label class="btn btn-wide btn-secondary {%  if good.calculate_method == 0 %} active {% endif %}">
                <input type="radio" name="calculate_method" value="dt_import" {%  if good.calculate_method == 0 %} checked="checked" {% endif %}>
                По дате производства(импорта)
              </label>
              <label class="btn btn-wide btn-secondary {%  if good.calculate_method == 1 %} active {% endif %}">
                <input type="radio" name="calculate_method" value="dt_sent" {%  if good.calculate_method == 1 %} checked="checked" {% endif %}>
          ...
          <div class="form-group mt-2">
            <div class="controls">
              <label class="form-label">{{ t._("Дата:") }}</label>
              <input type="text" name="good_date" id="good_date" data-provide="datepicker"
                     ...
                     class="form-control datepicker" placeholder="{{ date('d.m.Y') }}"
                     value="{{ date("d.m.Y", good.date_import) }}" autocomplete="off">
            </div>
          </div>
          ...
          <button type="submit" class="btn btn-success">{{ t._("edit-good") }}</button>
        </div>
      </div>
    </div>
  </div>
