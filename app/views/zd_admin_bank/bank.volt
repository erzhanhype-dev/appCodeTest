...
<form method="get" action="/zd_admin_bank/bank" class="card mt-3">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-2">
                ...
                <input name="idnum" type="number" class="form-control" id="searchJDBankByIdnum" value="{{ filters.idnum }}">
            </div>
            <div class="col-2">
                <label><b>Поиск по Референс:</b></label>
                <input name="reference" type="text" class="form-control" id="searchJDBankByReference" value="{{ filters.reference }}">
            </div>
            <div class="col-2">
                <label><b>Поиск по Год:</b></label>
                <select name="year" class="form-control" id="searchJDBankByYear">
                    {% for y in years %}
            ...
            <div class="col-2">
                ...
                <select name="status" class="form-control" id="searchJDBankByStatus">
            ...
            <div class="col-2">
                <label><b>Поиск по IBAN получателя: </b></label>

                <select name="iban_to[]" class="form-control selectpicker" multiple size="6" id="searchJDBankByIbanTo">
                    {% for iban in ibans %}
                        <option value="{{ iban }}"
                                {% if filters.iban_to is defined
                                    and (filters.iban_to is iterable)
                                    and (iban in filters.iban_to) %}
                        selected
                                {% endif %}>
                            {{ iban }}
                        </option>
            ...
            <div class="col-auto mt-4">
                ...
                <a href="/zd_admin_bank/bank?clear_filters=1" class="btn btn-warning">{{ t._('Сбросить фильтр') }}</a>

                <a href="#" class="btn btn-success export_jd_bank_btn">
                    <i class="fa fa-file-excel-o export_jd_bank_icon" aria-hidden="true"></i>
                    <span class="spinner-border spinner-border-sm" id="export_jd_bank_spinner" style="display:none"></span>
                    {{ t._("Выгрузить") }}
                </a>
            </div>
        </div>
...
<div class="card mt-3">
    ...
    <div class="card-body">
        <table class="table table-response bank-table">
            <thead>
            <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {% if auth is defined and (auth.isModerator() or auth.isSuperModerator() or auth.isAdminSoft() or auth.isAdmin()) %}
                                {% set disabled_btn = '' %}
                            {% endif %}
                            <a href="/zd_admin_bank/set/{{ r.id }}" class="btn btn-danger" {{ disabled_btn }}>
